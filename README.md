...
The DS33231 modules sometimes come with onboard circuitry to charge the backup battery. If not using a rechargeable battery this must be deactivated, e.g. by destroying a tiny diode next to the battery holder as explained elsewhere. Note that this project uses the EEPROM (typically an AT24C32 chip) which is preent on many DS3231-based RTC modules. If using a module without the EEPROM you would have to deactive that functionality (jumper on Arduino pin 3), but then it can't remember the countdown time if power was removed.
